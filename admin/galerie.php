<?php 
  ...
  if (isset($_POST['btn_ajout'])) {
    ...
    if($id==""){
      $sql='INSERT INTO galerie(id,nom,description,photo) VALUES ("",:nom,:descrip,:photo)';
      $stmt=$bdd->prepare($sql);
    ...
    /*--====================  Sinon l'id c'est une modification  =============-->*/
     ...
     $sql='UPDATE galerie SET nom=:nom, description=:descrip,photo=:photo WHERE id = :id';
     $stmt=$bdd->prepare($sql);
  ...
  <div class="container">
        ...
        <table class="table table-bordrered" width="50%">
          ...
          <?php foreach ($realisations as $real) : $desc=substr($real->description,0,50).'...' ; ?>
            <tr>
              <td><img src="img/gallery/<?=$real->photo; ?>" height="70" width="70"></td>
              <td><?=$real->nom ;?></td>
              <td><?=$desc;?></td>
              <td>
